...
from MythosEngine.config.config import load_note_templates
...
class BrowseView(QWidget):
    def __init__(self, parent, config):
        ...
        # --- Top Bar: Row 1 — Filters (Folder, Tag, Template) ─────────────
        filter_row = QFrame(self)
        filter_layout = QHBoxLayout(filter_row)
        filter_layout.setContentsMargins(8, 6, 8, 2)
        filter_layout.setSpacing(6)

        lbl_folder = QLabel("Folder:")
        lbl_folder.setProperty("role", "filter-label")
        filter_layout.addWidget(lbl_folder)
        self.folder_menu = QComboBox()
        self.folder_menu.setMinimumWidth(120)
        filter_layout.addWidget(self.folder_menu)

        lbl_tag = QLabel("Tag:")
        lbl_tag.setProperty("role", "filter-label")
        filter_layout.addWidget(lbl_tag)
        ...
        self.tag_logic.setMaximumWidth(60)
        filter_layout.addWidget(self.tag_logic)

        filter_layout.addSpacing(12)

        lbl_tmpl = QLabel("Template:")
        lbl_tmpl.setProperty("role", "filter-label")
        filter_layout.addWidget(lbl_tmpl)
        ...
        self.template_dropdown.setMinimumWidth(100)
        filter_layout.addWidget(self.template_dropdown)
        self.template_desc = QLabel("")
        self.template_desc.setProperty("role", "muted")
        filter_layout.addWidget(self.template_desc)
        ...
        self.fav_filter_cb = QCheckBox("Favorites only")
        ...
        action_layout.setContentsMargins(8, 2, 8, 6)
        action_layout.setSpacing(4)

        def _tb(label, primary=False, danger=False):
            """Helper to make a toolbar-styled button."""
            btn = QPushButton(label)
            if primary:
                btn.setProperty("toolbar-primary", "true")
            elif danger:
                btn.setProperty("toolbar-danger", "true")
            else:
                btn.setProperty("toolbar", "true")
            return btn

        self.create_btn = _tb("Create", primary=True)
        action_layout.addWidget(self.create_btn)
        self.import_btn = _tb("Import")
        action_layout.addWidget(self.import_btn)
        self.export_btn = _tb("Export")
        action_layout.addWidget(self.export_btn)

        action_layout.addSpacing(8)

        self.tag_btn = _tb("Tag")
        action_layout.addWidget(self.tag_btn)
        self.move_btn = _tb("Move")
        action_layout.addWidget(self.move_btn)
        self.star_btn = _tb("Favorite")
        action_layout.addWidget(self.star_btn)
        self.undo_move_btn = _tb("Undo")
        action_layout.addWidget(self.undo_move_btn)

        action_layout.addSpacing(8)

        self.delete_btn = _tb("Delete", danger=True)
        ...
        self.search_type.setMaximumWidth(80)
        self.search_btn = QPushButton("Search")
        self.search_btn.setProperty("toolbar-primary", "true")
        self.clear_search_btn = QPushButton("Clear")
        self.clear_search_btn.setProperty("toolbar", "true")
        ...
        # Primary actions (edit/save/cancel)
        self.edit_btn = QPushButton("Edit")
        self.edit_btn.setProperty("toolbar-primary", "true")
        ctrl_layout.addWidget(self.edit_btn)

        self.save_btn = QPushButton("Save")
        self.save_btn.setProperty("green", "true")
        self.save_btn.setStyleSheet("")  # let theme handle it
        ctrl_layout.addWidget(self.save_btn)

        self.cancel_btn = QPushButton("Cancel")
        self.cancel_btn.setProperty("toolbar", "true")
        ...
        self.summarize_btn.setProperty("toolbar", "true")
        ctrl_layout.addWidget(self.summarize_btn)

        self.history_btn = QPushButton("History")
        self.history_btn.setProperty("toolbar", "true")
        ctrl_layout.addWidget(self.history_btn)

        self.links_btn = QPushButton("Links")
        self.links_btn.setProperty("toolbar", "true")
        ctrl_layout.addWidget(self.links_btn)

        self.metadata_btn = QPushButton("Metadata")
        self.metadata_btn.setProperty("toolbar", "true")
        ctrl_layout.addWidget(self.metadata_btn)

        self.show_source_btn = QPushButton("Source")
        self.show_source_btn.setProperty("toolbar", "true")
    ...
    def _on_tab_changed(self, idx):
        ...
        # Update button state based on edit mode
        if widget and hasattr(widget, "editor") and not widget.editor.isReadOnly():
            self.enable_buttons(save=True, cancel=True, edit=False)
        else:
            self.enable_buttons(save=False, cancel=False, edit=True)
        # Update metadata display for the active tab
