"""
export_data.py — Export or restore all Ward DND AI user data.
...
- logs/audit.log
- Ward_DND_AI/ai/ai_usage_log.csv
- Global user/group data (~/.ward_dnd_ai/)
...
def _load_vault_path() -> Path:
    ...
    settings = PROJECT_ROOT / "Ward_DND_AI" / "config" / "settings.json"
    if settings.exists():
...
def export_data(output_dir: Path) -> Path:
    ...
    zip_path = output_dir / f"ward_dnd_export_{timestamp}.zip"

    print("Exporting Ward DND AI data...")
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        # 3. settings.json
        settings_file = PROJECT_ROOT / "Ward_DND_AI" / "config" / "settings.json"
        ...
        # 5. AI usage log
        ai_log = PROJECT_ROOT / "Ward_DND_AI" / "ai" / "ai_usage_log.csv"
        if ai_log.exists():
            zf.write(ai_log, "logs/ai_usage_log.csv")
            file_count += 1

        # 6. Global user/group data
        global_dir = Path("~/.ward_dnd_ai").expanduser()
...
def restore_data(zip_path: Path, target: Path) -> None:
    ...
    print("  2. Copy global/ files to ~/.ward_dnd_ai/ if needed")
    print("  3. Launch the app")


def main():
    parser = argparse.ArgumentParser(description="Ward DND AI data export/restore tool")
