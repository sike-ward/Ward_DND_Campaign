...
def highlight_markdown(text_widget):
    ...
    for m in re.finditer(r'\*\*(.+?)\*\*', content):
        s = f"1.0+{m.start(1)}c"; e = f"1.0+{m.end(1)}c"
        tv.tag_add("bold", s, e)
    ...
    for a, b, emoji in repls:
        s = f"1.0+{a+offset}c"; e = f"1.0+{b+offset}c"
    ...
    tv.tag_configure("code",      font=("Consolas", 12),
                                 background="#2e3440",
                                 foreground="#88c0d0")
    tv.tag_configure("blockquote",font=("Consolas", 12, "italic"),
                                 foreground="#657b83")
    tv.tag_configure("list",      foreground="#d8dee9")
    tv.tag_configure("link",      foreground="#81a1c1",
                                 underline=True)
    tv.tag_configure("icon",      font=("Consolas", 12))
    for lvl,(size,color) in enumerate([
        (20, "#bf616a"), (18, "#d08770"), (16, "#ebcb8b"),
        (14, "#a3be8c"), (13, "#88c0d0"), (12, "#5e81ac")
    ], start=1):
        tv.tag_configure(
            f"heading{lvl}",
