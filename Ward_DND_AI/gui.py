...
from ai_engine import LoreAI
from config import Config
from tooltip import ToolTip
from utils import make_title_case_filename, get_note_names, auto_link_notes, get_all_folders
from ai_engine import count_tokens  # after LoreAI import
import re
from PIL import Image
from tkhtmlview import HTMLLabel
import markdown2
from utils import highlight_markdown
from utils import make_title_case_filename, get_note_names, auto_link_notes, get_all_folders, highlight_markdown
...
class LoreMainApp(ctk.CTk):
    def __init__(self):
        ...
        self.bind("<Control-p>", lambda e: self.preview_in_browser(self.output_box.get("1.0", "end")))
        ...
        self.status_label = ctk.CTkLabel(self, textvariable=self.status_var, font=("Segoe UI", 18, "bold"))
    ...
    def _setup_menubar(self):
        ...
        filemenu.add_command(label="Save Note", command=self.save_note, accelerator="Ctrl+S")
        filemenu.add_command(label="Export Note", command=self.browse_export_note, accelerator="Ctrl+Shift+S")
        filemenu.add_command(label="Import Note", command=self.browse_import_note, accelerator="Ctrl+I")
        filemenu.add_separator()
        filemenu.add_command(label="Ask AI", command=self.ask_question, accelerator="Ctrl+Enter")
        filemenu.add_command(label="Preview", command=lambda: self.preview_in_browser(self.output_box.get("1.0", "end")), accelerator="Ctrl+P")
        filemenu.add_command(label="Next Tab", command=self.next_tab, accelerator="Ctrl+Tab")
        filemenu.add_command(label="Previous Tab", command=self.prev_tab, accelerator="Ctrl+Shift+Tab")
        filemenu.add_command(label="Show Help/About", command=lambda: self.tabview.set("Help / About"), accelerator="F1")
        filemenu.add_separator()
        ...
        editmenu.add_command(label="Undo", command=lambda: self.try_undo(), accelerator="Ctrl+Z")
        editmenu.add_command(label="Redo", command=lambda: self.try_redo(), accelerator="Ctrl+Y")
        editmenu.add_separator()
        editmenu.add_command(label="Cut", command=lambda: self.focus_get().event_generate("<<Cut>>"), accelerator="Ctrl+X")
        editmenu.add_command(label="Copy", command=lambda: self.focus_get().event_generate("<<Copy>>"), accelerator="Ctrl+C")
        editmenu.add_command(label="Paste", command=lambda: self.focus_get().event_generate("<<Paste>>"), accelerator="Ctrl+V")
        editmenu.add_separator()
        editmenu.add_command(label="Find", command=self.find_text, accelerator="Ctrl+F")
        editmenu.add_command(label="Replace", command=self.replace_text, accelerator="Ctrl+H")
        menubar.add_cascade(label="Edit", menu=editmenu)

        # --- AI Menu ---
        aimenu = tk.Menu(menubar, tearoff=0)
        aimenu.add_command(label="Ask", command=self.ask_question, accelerator="Ctrl+Enter")
    ...
    def _setup_ask_tab(self):
        ...
        ctk.CTkLabel(main, text="Ask about your world:", font=("Segoe UI", 15)).grid(row=0, column=0, sticky="w", padx=6, pady=(8, 2))
        self.input_box = ctk.CTkTextbox(main, height=44, font=("Consolas", 14), undo=True)
                # place the prompt textbox
        self.input_box.grid(row=1, column=0, sticky="nsew", padx=6, pady=(0,4))

        self.output_box = ctk.CTkTextbox(main, font=("Consolas", 13), wrap="word", state="disabled", undo=True)


        self.input_box.bind("<Control-Return>", lambda e: self.ask_question())
        self.input_box.bind("<Control-s>", lambda e: self.save_note())
        self.input_box.bind("<<Modified>>", lambda e: self.highlight_markdown(self.input_box))
        ...
        btn_split = ctk.CTkButton(btns, text="Split & Save", command=self.split_and_save_multiple, height=36)
        btn_split.grid(row=0, column=2, padx=2, pady=2, sticky="ew")
        ToolTip(btn_split, "Split the response into multiple notes (if multiple headings)")
        ...
        btn_preview = ctk.CTkButton(
            ...
            command=lambda: self.preview_in_browser(
                auto_link_notes(self.output_box.get("1.0", END), get_note_names(self.config.VAULT_PATH))
            ),
            height=36,
        )
        ...
        btn_clear = ctk.CTkButton(btns, text="Clear History", command=self.clear_history, height=36)
        btn_clear.grid(row=0, column=4, padx=2, pady=2, sticky="ew")
        ToolTip(btn_clear, "Clear the conversation history for this session")

        ctk.CTkLabel(main, text="AI Response / Note Preview:", font=("Segoe UI", 13, "italic")).grid(
            row=3, column=0, sticky="w", padx=6, pady=(0, 3)
        )
        self.output_box = ctk.CTkTextbox(main, font=("Consolas", 13), wrap="word", state="disabled")
        self.output_box.grid(row=3, column=0, sticky="nsew", padx=6, pady=(0, 10))

        ctk.CTkLabel(main, text="Conversation History:", font=("Segoe UI", 13)).grid(row=4, column=0, sticky="w", padx=6, pady=(0, 2))
        history_frame = ctk.CTkFrame(main)
        history_frame.grid(row=4, column=0, sticky="nsew", padx=6, pady=(0, 8))
        history_frame.rowconfigure(0, weight=1)
        history_frame.columnconfigure(0, weight=1)
        self.history_text = ctk.CTkTextbox(history_frame, font=("Consolas", 11), wrap="word", state="disabled")
    ...
    def _setup_browse_tab(self):
        ...
        ctk.CTkLabel(left, text="Folder:", font=("Segoe UI", 13))\
            .grid(row=0, column=0, sticky="w", padx=(6,2), pady=(6,2))
        ctk.CTkOptionMenu(left,
            variable=self.folder_filter_var,
            values=folders,
            width=160
        ).grid(row=0, column=1, sticky="ew", padx=(2,8), pady=(6,2))
        self.folder_filter_var.trace_add("write", lambda *a: self._browse_filter_notes())

        ctk.CTkLabel(left, text="Search:", font=("Segoe UI", 13))\
            .grid(row=0, column=2, sticky="e", padx=(8,2), pady=(6,2))
        self.browse_search_var = tk.StringVar()
        ctk.CTkEntry(left, textvariable=self.browse_search_var)\
            .grid(row=0, column=3, sticky="ew", padx=(2,6), pady=(6,2))
        ...
        ctk.CTkLabel(left, text="Tag:", font=("Segoe UI", 13))\
            .grid(row=1, column=0, sticky="w", padx=(6,2), pady=(0,2))
        ...
        self.browse_listbox.grid(row=2, column=0, columnspan=4,
                                 sticky="nsew", padx=6, pady=(0,8))
        scroll = ctk.CTkScrollbar(
            left, orientation="vertical",
            command=self.browse_listbox.yview
        )
        scroll.grid(row=2, column=4, sticky="ns", pady=(0,8))
        self.browse_listbox.configure(yscrollcommand=scroll.set)
        self.browse_listbox.bind("<<ListboxSelect>>",
                                 lambda e: self.browse_preview_note())

        # Left-pane action buttons (row 3+)
        btns = ctk.CTkFrame(left)
        btns.grid(row=3, column=0, columnspan=4,
                  sticky="ew", padx=6, pady=(0,8))
        btns.columnconfigure((0,1), weight=1)
        ctk.CTkButton(btns,
            text="Open in VS Code",
            command=self.browse_open_in_vscode,
            height=36
        ...
        ctk.CTkButton(left, text="New Note",
            command=self.browse_new_note
        ).grid(row=4, column=0, columnspan=4,
               sticky="ew", padx=6, pady=(0,8))
        ctk.CTkButton(left, text="AI Summarize",
            command=self.browse_ai_summarize
        ...
        ctk.CTkLabel(right, text="Preview:", font=("Segoe UI", 13))\
        ...
        self.browse_preview_box.bind(
            "<<Modified>>",
            lambda e: highlight_markdown(self.browse_preview_box)
        )
        ...
        self.cancel_note_btn.grid(
            row=5, column=0, sticky="e", padx=6, pady=(0,2)
        )
        self.cancel_note_btn.configure(state="disabled")

        # Export / Import
        ctk.CTkButton(
            right, text="Export", command=self.browse_export_note
        ).grid(row=6, column=0, sticky="ew", padx=(6,2), pady=(0,8))
        ctk.CTkButton(
            right, text="Import", command=self.browse_import_note
        ).grid(row=6, column=1, sticky="ew", padx=(2,6), pady=(0,8))
        ...
        def on_browse_right_click(event):
            ...
            if selection >= 0:
                ...
                menu.add_command(label="Open in VS Code", command=self.browse_open_in_vscode)
                menu.add_command(label="Open in Obsidian", command=self.browse_open_in_obsidian)
    ...
    def _browse_filter_notes(self):
        ...
        query = self.browse_search_var.get().lower() if hasattr(self, "browse_search_var") else ""
    ...
    def browse_new_note(self):
        ...
        folder_menu = ctk.CTkOptionMenu(dialog, variable=folder_var, values=folders, width=340)
        ...
        def do_create():
            ...
            folder_path = os.path.join(self.config.VAULT_PATH, folder) if folder else self.config.VAULT_PATH
            ...
            try:
                ...
                self._browse_load_notes()
                self._browse_filter_notes()
                self.status_var.set(f"New note created: {os.path.join(folder, filename + '.md')}")
                dialog.destroy()
        ...
        btns.pack(pady=16)
        ctk.CTkButton(btns, text="Create", command=do_create, width=80).grid(row=0, column=0, padx=4)
        ctk.CTkButton(btns, text="Cancel", command=dialog.destroy, width=80).grid(row=0, column=1, padx=4)
    ...
    def enable_browse_edit(self):
        ...
        self.status_var.set("Editing enabled. Make changes and click 'Save Changes' to save.")
    ...
    def browse_export_note(self):
        ...
        dest = fd.asksaveasfilename(title="Export Note", defaultextension=".md", initialfile=default_name)
        if dest:
            try:
                with open(src, "r", encoding="utf-8") as fsrc, open(dest, "w", encoding="utf-8") as fdest:
    ...
    def browse_import_note(self):
        files = fd.askopenfilenames(title="Import Note(s)", filetypes=[("Markdown Files", "*.md"), ("All Files", "*.*")])
    ...
    def browse_delete_note(self):
        ...
        if not sel:
            self.status_var.set("No note selected.")
            return
        confirm = messagebox.askyesno("Delete Note", f"Are you sure you want to permanently delete '{sel}'?")
        if not confirm:
            return
    ...
    def browse_preview_note(self):
        ...
        if not sel:
            ...
            self.browse_preview_box.grid(row=1, column=0, columnspan=2, sticky="nsew", padx=6, pady=(0,4))
            return
    ...
    def _setup_summarize_tab(self):
        ...
        ctk.CTkLabel(main, text="Summarize a folder of notes:", font=("Segoe UI", 15)).grid(
            row=0, column=0, sticky="w", padx=6, pady=(14, 2)
        )
        btn_summarize = ctk.CTkButton(main, text="Summarize Folder", command=self.summarize_folder, height=36)
        btn_summarize.grid(row=1, column=0, sticky="ew", padx=6, pady=(0, 8))
        ToolTip(btn_summarize, "Summarize all notes in the selected folder (great for campaign recaps!)")
        self.summarize_output = ctk.CTkTextbox(main, font=("Consolas", 13), wrap="word", state="disabled")
    ...
    def summarize_folder(self):
        ...
        def do_summarize():
            ...
            try:
                ...
                self.status_var.set(f"Summarized {len(texts)} notes in {folder_path}. Session tokens: {self.session_tokens}")
                if hasattr(self, "token_label"):
                    self.token_label.configure(text=f"Session tokens used: {self.session_tokens}")
                dialog.destroy()
        ...
        ctk.CTkButton(dialog, text="Summarize", command=do_summarize, width=80).pack(pady=16)
        dialog.transient(self)
        dialog.grab_set()
        self.wait_window(dialog)
    ...
    def _setup_help_tab(self):
        ...
        box = ctk.CTkTextbox(help_frame, font=("Segoe UI", 13), wrap="word", state="normal", width=800, height=400)
    ...
    def split_and_save_multiple(self):
        ...
        def do_save_all():
            ...
            for part in parts:
                ...
                try:
                    ...
                    log_exception(f"Failed to save split note: {filename}", e)
                    errors.append(filename)

            if errors:
                messagebox.showwarning("Partial Save", f"Some notes failed to save: {', '.join(errors)}")
            else:
        ...
        ctk.CTkButton(dialog, text="Save All", command=do_save_all, width=90).pack(pady=16)
        dialog.transient(self)
        dialog.grab_set()
        self.wait_window(dialog)
    ...
    def _setup_random_generator_tab(self):
        ...
        main = ctk.CTkFrame(tab)
        main.grid(row=0, column=0, sticky="nsew", padx=12, pady=12)
        ctk.CTkLabel(main, text="Random Generator Features Coming Soon", font=("Segoe UI", 15)).pack(pady=30)
    ...
    def _setup_timeline_tab(self):
        ...
        main = ctk.CTkFrame(tab)
        main.grid(row=0, column=0, sticky="nsew", padx=12, pady=12)
        ctk.CTkLabel(main, text="Timeline Features Coming Soon", font=("Segoe UI", 15)).pack(pady=30)
    ...
    def _setup_campaign_settings_tab(self):
        ...
        main = ctk.CTkFrame(tab)
        main.grid(row=0, column=0, sticky="nsew", padx=12, pady=12)
        ctk.CTkLabel(main, text="Campaign Settings Features Coming Soon", font=("Segoe UI", 15)).pack(pady=30)
    ...
    def _setup_token_tab(self):
        ...
        ctk.CTkLabel(main, text="Token Usage Info", font=("Segoe UI", 15, "bold")).pack(pady=(0, 12))
        self.token_label = ctk.CTkLabel(main, text=f"Session tokens used: {self.session_tokens}", font=("Consolas", 13))
        ...
        ctk.CTkButton(main, text="Set Limit", command=self._update_token_limit).pack(pady=4)
    ...
    def find_text(self):
        ...
        def do_find():
            ...
            if start:
                ...
                widget.tag_config("find_highlight", background="#ffe066", foreground="#23272e")
                widget.see(start)
    ...
    def replace_text(self):
        ...
        ctk.CTkButton(dialog, text="Replace All", command=lambda: do_replace()).pack(pady=(2, 12))
        dialog.transient(self)
        dialog.grab_set()
        self.wait_window(dialog)
    ...
    def highlight_markdown(self, textbox):
        ...
        textbox.tag_config("header", foreground="#63aaff", font=("Segoe UI", 14, "bold"))
        # Bold (**)
        ...
        textbox.tag_config("code", background="#282C34", foreground="#FFD580", font=("Consolas", 12, "bold"))



    def toggle_browse_preview(self):
        # get the single selected note (or empty)
        sel_list = self._browse_get_selection()
        sel = sel_list[0] if sel_list else ""
        # read its content
        path = os.path.join(self.config.VAULT_PATH, sel)
        ...
        if self.browse_preview_mode == "text":
            ...
            # create or update the HTMLLabel
            if getattr(self, "browse_html_preview", None) is None:
                self.browse_html_preview = HTMLLabel(
                    self.browse_preview_box.master,
                    html=html,
                    background="#23272e",
                ...
                self.browse_html_preview.set_html(html)
                self.browse_html_preview.grid()
            # ensure the textbox remains visible and editable
            self.browse_preview_box.grid()
            self.browse_preview_box.configure(state="normal")
            ...
            if getattr(self, "browse_html_preview", None):
                self.browse_html_preview.grid_remove()
            self.browse_preview_box.grid()
            self.browse_preview_box.configure(state="normal")
